              ...
              .then((repos) => {
                ...
                repos.forEach((repo) => {
                  const repoItem = `
                          ...
                          repo.description ? `${repo.description}, and it ` : ""
                        }has ${repo.stargazers_count} stars.</p>
												<p class="text-gray-600 dark:text-gray-200">Created on <span class="italic">${repo.created_at.slice(
                          0,
                          10
                        )}</span>, and last updated on <span class="italic">${repo.updated_at.slice(
                    0,
                    10
                  )}</span>.</p>
												<a href="${
                          repo.html_url
                        }" target="blank" class="underline decoration-transparent text-blue-600 dark:text-blue-400 hover:decoration-inherit transition duration-300 ease-in-out">View Repository</a>
										</li>
								`;
